...
/*   Updated: 2024/08/27 07:56:53 by ychagri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mutexes(t_data *data)
{
	...
	while (index < data->number_of_philosophers)
	{
		...
		if (pthread_mutex_init(&data->philos[index].l_fork, NULL))
			return (ft_perror("pthread_mutex_init() has failed !!"), 1);
		data->philos[index].r_fork = &data->philos[(index + 1)
			% data->number_of_philosophers].l_fork;
		index++;
	}
	return (0);
}
...
void	write_lock(t_philo *philo, char *msg)
{
	...
	printf("=> %llu ms philo %d %s.\n", timeofday(philo->data->start_time),
		philo->number, msg);
...
{
	if (philo->data->timing.number_of_times_each_philosopher_must_eat
		&& philo->meals_ate
		== philo->data->timing.number_of_times_each_philosopher_must_eat)
	{
